...
export default async function handler(req, res) {
  ...
  try {
    ...
    const orderReference = referenceId || `PEDIDO-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    const amountInCents = Math.round(amount * 100);

    const payload = {
      productHash: process.env.VITE_PARADISE_PRODUCT_HASH || "prod_c0f05a6a02eb0537",
    ...
    console.log(`Gerando PIX Paradise Pags - Valor: ${amount} - Pedido: ${orderReference}`);
